...
export class CommentProcessingService {
	...
	public processRawCommentPipeline = async (
		...
		// 1st, Save the raw comment to our database. This gives us a complete record.
		await insertComment({
		...
		// 2nd, Cache the text for context lookups of its children.
		...
		if (comment.text.length < 50) {
			return; // Skip analysis for very short comments
		}

		// 1. Check if already analyzed for insight
		if (await findInsightBySourceCommentId(comment.sourceCommentId)) {
			return;
		}

		// 2. Keyword pre-filter - only pass comments that "look" sentiment heavy to the AI
		if (!this.passesKeywordCheck(comment.text)) {
			return; // Skip comments without keywords
		}

		// 3. If it passes, get context and analyze
		console.log(
		...
		// 4. If insight found, create embedding and insert
